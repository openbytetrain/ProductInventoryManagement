...
import com.bytetrain.prodinv.config.KafkaProperties
...
class ProductinventoryKafkaResourceIT {
    ...
    fun setup() {
        ...
        val kafkaResource = ProductinventoryKafkaResource(kafkaProperties)
    ...
    fun producesMessages() {
        restMockMvc.perform(post("/api/productinventory-kafka/publish/topic-produce?message=value-produce"))
    ...
    fun consumesMessages() {
        ...
        val mvcResult = restMockMvc.perform(get("/api/productinventory-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
